...
//前后指针法快速排序
//prev:上一个小于基准值的位置,cur:上一个小于基准值的位置
//当cur走到一个小于基准值的位置，则：判断prev和cur是否连续
//如果连续：区间[prev,cur]的值都是小于基准值的值，更新prev，cur
//如果不连续：区间[prev,cur]的值，有大于等于基准值的值，跟新prev，数据交换，更新cur
...
//归并：合并有序的子序列，产生更大的有序的子序列
	  ...
	  //合并：相邻有序的子序列进行合并，排序的过程中，需要o（n）的辅助空间
void merge(int* arr, int begin, int mid, int end, int* tmp)
{
	//递增
	//字区间：[begin,mid],[mid+1],end]
	int begin1 = begin;
	int end1 = mid;
	int begin2 = mid + 1;
	int end2 = end;
	...
	//判断是否有为合并的元素，如果有剩余元素，把剩余元素全部拷到tmp的相应位置
	if (begin1 <= end1)
...
//非递归，归并排序
//要合并的第一个序列begin: idx ,     end: idx + step - 1
//要合并的第二个序列begin: idx + step,    end: idx + 2 * step - 1  
//idx += 2 * step
//step*=2
